...
The first thing we want to do is set up our basic Express server. Most of this process is covered in the [first part of this series]('/your-first-express-server), so I won't go into too much detail on this here...
...
![Well, We're Waiting!](https://giphy.com/embed/oT7ATDykMidsk)
...
We're going to put the definitions for all for all of the keys of our `Puppy` model in the `new Schema({})` assigned to `puppySchema`. MongoDB offers all of the standard data types, and Mongoose provides validations for these types. We'll be exploring a few different data types and validations.

We want all of our `Puppies` to have a `name`, `age`, and `breed`. We're also going to give them each `Boolean` values of `cute`,`well-behaved`, and `adopted`.
